...
class App_Shortcode_WeeklySchedule extends App_Shortcode {
	...
	public function process_shortcode ($args=array(), $content='') {
        ...
        if (!$has_worker && !empty($require_provider)) {
			...
			if(!empty($_REQUEST['reservation_code'])){}else{
...
class App_Shortcode_Services extends App_Shortcode {
	...
	public function process_shortcode ($args=array(), $content='') {
		...
		$s .= '<div class="appointments-field appointments-reservation_code-field"><label for="appointments-reservation_codec0dd276d7b88a1e0d4fe71c58c963f5f" style="display: block;margin-top: 10px;margin-bottom: 10px;"><span>Reservation Code (If you have) </span></label><input type="text" id="appointments-reservation_codec0dd276d7b88a1e0d4fe71c58c963f5f" class="appointments-field-entry appointments-reservation_code" name="reservation_code" value="'.$_REQUEST['reservation_code'].'"></div>';		
		...
		$script .= "var reservation_code=$('.appointments-reservation_code').val();";
		...
		if ( $autorefresh ) {
		$script .= "if(reservation_code != null ){";
		$script .= "window.location.href='".$href."'.replace(/__selected_service__/, selected_service)+'&attendees='+attendees_num+'&reservation_code='+reservation_code+''";
		$script .= "}else{";
...
class App_Shortcode_Paypal extends App_Shortcode {
	...
	public function process_shortcode ($args=array(), $content='') {
		...
		$form .= '<input class="app_custom_reservation_code" type="hidden" name="app_custom_reservation_code" value="" />';
		$form .= '<input class="app_cid" type="hidden" name="cid" value="" />';
		$form .= '<input class="app_custom_save_reserved" type="hidden" name="custom_save_reserved" value="" />';
		...
		$form .= '<img src="/wp-content/uploads/2015/10/PayPal-Secure.jpg">';
...
class App_Shortcode_Confirmation extends App_Shortcode {
	...
	public function process_shortcode ($args=array(), $content='') {
		...
		if(!empty($_REQUEST['reservation_code'])){
			...
			$rev = $_REQUEST['reservation_code'];
			global $wpdb;
		$app_appointments_taken = $wpdb->get_var( "SELECT COUNT(*) FROM wp_app_appointments WHERE reservation_code LIKE '$rev' AND `status` LIKE  'reserved'" );
		if($app_appointments_taken>=$_REQUEST['attendees'])	{
		$ret .= '';
		...
		$ret .= '<input type="hidden" name="business" value="' . esc_attr($appointments->options['merchant_email']) . '" />';
		$ret .= '<input type="hidden" name="cmd" value="_xclick">';
		$ret .= '<input type="hidden" class="app_item_name" name="item_name" value="' . $item_name . '" />';
		$ret .= '<input type="hidden" name="no_shipping" value="1" />';
		$ret .= '<input type="hidden" name="currency_code" value="' . $appointments->options['currency'] .'" />';
		$ret .= '<input type="hidden" name="return" value="' . get_site_url() . '" />';
		$ret .= '<input type="hidden" name="cancel_return" value="' . $cancel_return . '" />';
		$ret .= '<input type="hidden" name="notify_url" value="' . admin_url('admin-ajax.php?action=app_paypal_ipn') . '" />';
		$ret .= '<input type="hidden" name="src" value="0" />';
		$ret .= '<input class="app_custom_res" type="hidden" name="custom" value="'.$_REQUEST['attendees'].'|'.$_REQUEST['reservation_code'].'|pending|reserved" />';
		//$ret .= '<input class="app_cid" type="hidden" name="cid" value="" />';
		//$ret .= '<input class="app_custom_save_reserved" type="hidden" name="custom_save_reserved" value="" />';
		$pf = $_REQUEST['attendees'] * 20;
		$ret .= '<input class="app_amount" type="hidden" name="amount" value="'.$pf.'" />';
		
		$ret .= '<input class="app_submit_btn_not" type="submit" name="submit_btn" value="Please confirm $'.$pf.' USD payment for Default Service"> ';
		$ret .= '<img style="display:none" alt="" border="0" width="1" height="1" src="https://www.paypal.com/en_US/i/scr/pixel.gif">';
		$ret .= '<img src="/wp-content/uploads/2015/10/PayPal-Secure.jpg">';
		$ret .= '</form>';
		$ret .= '';
		...
		$ret .='<script>
			...
			jQuery(document).ready(function($){
				//var res_attendees = '.$_REQUEST['attendees'].';
				$(\'input[type="radio"]\').click(function(){
        if($(this).attr("value")=="twenty"){
			var res_attendees = '.$_REQUEST['attendees'].' * 20;
			var res_val_att = "pending";
            $(".app_amount").val(res_attendees);
        }else if($(this).attr("value")=="full"){';
			if($_REQUEST['app_service_id']==1){
			$ret .='var res_attendees = '.$_REQUEST['attendees'].' * 30;';
			$ret .='var res_val_att = "active";';
			}elseif($_REQUEST['app_service_id']==2){
			$ret .='var res_attendees = '.$_REQUEST['attendees'].' * 65;';
			$ret .='var res_val_att = "active"';
				}
			
           $ret .=' $(".app_amount").val(res_attendees);
        }';
		
		    $ret .=' var sub_valu = "Please confirm $"+res_attendees+" USD payment for Default Service";';
           $ret .=' var sub_cus = "'.$_REQUEST['attendees'].'|'.$_REQUEST['reservation_code'].'|"+res_val_att+"|reserved";';
		   
           $ret .=' $(".app_submit_btn_not").val(sub_valu);
		   $(".app_custom_res").val(sub_cus);

    });
			})
			</script>';
		}elseif($app_appointments_taken==0 or $app_appointments_taken == NULL or empty($app_appointments_taken)){
			$ret .='<h1>Invalid reservation code.</h1>';
		}elseif($app_appointments_taken<=$_REQUEST['attendees']){
			$ret .='<h1>Only '.$app_appointments_taken.' is available.</h1>';
			
		}else{
			$ret .='<h1>It is not available. At the moment.</h1>';
			}
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-primary out_submit" data-loading-text="Loading..." autocomplete="off" data-dismiss="modal">Save changes</button>
      </div>
    </div>
  </div>
</div>';
		...
		$ret .='<div class="appointments-field appointments-price-main-field"><label for="appointments-attendeesnumberonlyc0dd276d7b88a1e0d4fe71c58c963f5f"><span>Are you paying for only part of your group?</span></label><input type="radio" id="reservation" class="appointments-field-entry appointments-reservation-field-entry" name="reservation" value="1"> Yes <input type="radio" id="reservation" class="appointments-field-entry appointments-reservation-field-entry" name="reservation" value="0" checked> No </div>';
		
		$ret .='<div class="appointments-field appointments-reservation-number-field" style="display:none; padding-top:10px;"><label for="appointments-attendeesnumberonlyc0dd276d7b88a1e0d4fe71c58c963f5f"><span>How many attendees are you paying for?</span></label><input type="text" id="reservation_number" class="appointments-field-entry appointments-reservation-number-field-entry" name="reservation" value=""></div>';
        ...
        if($(this).attr("value")=="1"){
            $(".appointments-reservation-number-field").show();
        }
        if($(this).attr("value")=="0"){
            $(".appointments-reservation-number-field").hide();
        }
    });
	$(".out_submit").click(function(){
	';
	//$ret .=$appointments->attendees_validate_fields($_REQUEST['attendees']);
	...
	$ret .='
		...
		$script .= '$(".app_submit_btn").click(function(){';
		$script .= 'var save_reserved = $(".app_custom_save_reserved").val();';
		$script .= 'var app_custom_reservation_code = $(".app_custom_reservation_code").val();';
		$script .= 'var app_cid = $(".app_cid").val();';
		$script .= 'var custom_save_reserved = {action: "app_custom_save_reserved", save_reserved: save_reserved, app_cid: app_cid, app_custom_reservation_code: app_custom_reservation_code,  nonce: "'. wp_create_nonce() .'"};';

		$script .= '$.post(_appointments_data.ajax_url, custom_save_reserved, function(response) {
						$(".wait_img").remove();
						if ( response && response.error ) {
							alert(response.error);
						}
		...
		$script .= 'var reservation = $(".appointments-reservation-field-entry[name=reservation]:checked").val();';
		
		$script .= 'var reservation_number = $(".appointments-reservation-number-field-entry").val();
		if(reservation==1){
			if(reservation_number == null || reservation_number== ""){
				alert("Please enter the rerservation number");
				die("click");
				}
			} 
		';
		...
		$script .= 'var post_data = {action: "post_confirmation", value: final_value, app_name: app_name, app_email: app_email, app_phone: app_phone, app_address: app_address, app_city: app_city, app_note: app_note, app_gcal: app_gcal, attendees: attendees, refund: refund, coupon: coupon, att_info: attendees_info, reservation: reservation, reservation_number: reservation_number,   nonce: "'. wp_create_nonce() .'"};';
		...
		$script .= '$.post(_appointments_data.ajax_url, post_data, function(response) {
						...
						if ( response && response.error ) {
							...
							$(".appointments-paypal").find(".app_custom").val(response.custom2);
							$(".appointments-paypal").find(".app_custom_reservation_code").val(response.reservation_code);
							$(".appointments-paypal").find(".app_custom_save_reserved").val(response.result);
							$(".appointments-paypal").find(".app_cid").val(response.cid);
