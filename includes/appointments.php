...
<div class="wrap">
	...
	function attendees_fields_form_backend($i, $name, $age, $weight, $insuran, $bk_select_provider, $service_id, $start, $end, $status){
			$output = '';
			$output = '<h3>Attendees Information - '.$i.'</h3>';
			...
			$output .= '<div>Status:</div><div><select name="bk_select_status_'.$i.'" id="bk_select_status_'.$i.'" >'.$this->status_bkand($status).'</select></div>';
			return $output;
		}
  ...
  <div class="panel panel-default">';
		foreach	($query as $q){
				$ret .='<div class="panel-heading" role="tab" style="padding: 5px 10px;" id="heading_'.$i.'">';
				$ret .='<h4 class="panel-title">';
				$ret .='<a style="display:block;" role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse_'.$i.'" aria-expanded="false" aria-controls="collapse_'.$i.'">'.$q->name.'<sup style="text-transform: lowercase;    padding: 5px 10px;    margin-left: 10px;    background-color: #337AB7;    border-radius: 5px 5px 5px 5px;    color: #fff;">'.$q->status.'</sup></a>';
				$ret .='</h4>';
				$ret .='</div>';
				$ret .=' <div id="collapse_'.$i.'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading_'.$i.'">';
				$ret .=' <div class="panel-body">';
				$ret.= $this->attendees_fields_form_backend($i, $q->name, $q->age, $q->weight, $q->insurance_waver, $q->worker, $q->service, $q->start, $q->end, $q->status);
				$ret .='</div>';		
				$ret .='</div>';		
				$i++;
			}
...
';	
	...
	function bk_attendees_fields_form_jquery($i){
		...
		$script .= 'var bk_select_status_'.$i.' = $("#bk_select_status_'.$i.'").val();';
		return $script;
		}
	...
	function bk_attendees_fields_ajax($cid){
		...
		foreach	($query as $q){
					if($i>1){
					$output .= '!';
					$output .= '"+bk_att_name_'.$i.'+"|"+bk_att_age_'.$i.'+"|"+bk_att_weight_'.$i.'+"|"+bk_insurance_waver_'.$i.'+"|"+bk_select_provider_'.$i.'+"|"+bk_select_status_'.$i.'+"|"+"'.$q->ID.'"+"';
...
function receive_paypal_2($custom){
	...
	"
		...
		//wp_mail('shimionson@gmail.com', 'Hello Testing', 'We are here for you.');
